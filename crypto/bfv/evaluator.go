...
// and ciphertexts that will be used for 中间值.
...
// getElemAndCheckBinary unwraps the elements from the operands and checks that the
// receiver has sufficiently large degree.
...
// 将 el0 和 el1 中的元素按照幂次进行 evaluate
...
// 对el0中的元素进行操作，将得到的结果返回到 elout 中
...
// 密文和明文相乘
// 密文是一个多项式数组，首先将每一个多项式转换成NTT格式，之后运用乘法依次和明文相乘，最后将结果依次进行INTT转换即可
...
// 明文和密文相乘，明文格式是PlaintextRingTone格式
func (eval *evaluator) mulPlaintextRingT(ct0 *Ciphertext, ptRt *PlaintextRingT, ctOut *Ciphertext) {
	...
	for i := range ct0.value {
		...
		for j := range ringQ.Modulus {
			...
			// Transforms the 明文 in the NTT domain of that qi
...
// relinearize is a method common to Relinearize and RelinearizeNew.
// It switches ct0 to the NTT domain, applies the keyswitch, and returns the result out of the NTT domain.
// 使用的是第一种重现性化的方法
...
// SwitchKeys applies the key-switching procedure to the ciphertext ct0 and returns the result in ctOut. It
// requires as an additional input a valid switching-key:
...
// SwitchKeysNew applies the key-switching procedure to the ciphertext ct0 and creates a new ciphertext
// to store the result. It requires as an additional input a valid switching-key:
// it must encrypt the target key under the public key under which ct0 is currently encrypted.
...
// As an additional input it requires a RotationKeys struct:
//
...
// InnerSum computes the inner sum of ct0 and returns the result in ctOut.
// It requires a rotation key storing all the left powers of two rotations.
