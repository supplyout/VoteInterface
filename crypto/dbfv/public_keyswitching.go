...
// PCKSProtocol 是一个用来存储联合公钥交换参数的结构
...
// NewPCKSProtocol creates a new PCKSProtocol object and will be used to re-encrypt a ciphertext
// ctx encrypted under a secret-shared key among j parties under a new collective public-key.
// NewPCKSProtocol 创建了一个新的PCKSPorotocol对象，并且该对象将被用来重加密一个密文ctx，该密文原本是用一个secret key相关的
// 联合公钥进行加密的
...
// GenShare is the first part of the unique round of the PCKSProtocol protocol.
...
// AggregateShares is the second part of the first and unique round of the PCKSProtocol protocol.
// Each party upon receiving the j-1 elements from the other parties computes :
//
