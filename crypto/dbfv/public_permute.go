...
func (pp *PermuteProtocol) GenShares(sk *ring.Poly, ciphertext *bfv.Ciphertext, crs *ring.Poly,
	permutation []uint64, share RefreshShare) {
...
func (pp *PermuteProtocol) Decrypt(ciphertext *bfv.Ciphertext, shareDecrypt RefreshShareDecrypt,
	sharePlaintext *ring.Poly) {
	pp.context.ringQ.Add(ciphertext.Value()[0], shareDecrypt, sharePlaintext)
}

// Permute decodes and re-encode (removing the error) the masked decrypted ciphertext with a permutation of the
// plaintext slots.
...
func (pp *PermuteProtocol) Recrypt(sharePlaintext *ring.Poly, crs *ring.Poly, shareRecrypt RefreshShareRecrypt,
...
func (pp *PermuteProtocol) Finalize(ciphertext *bfv.Ciphertext, permutation []uint64, crs *ring.Poly,
	share RefreshShare, ciphertextOut *bfv.Ciphertext) {
