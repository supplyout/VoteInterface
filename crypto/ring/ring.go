// Package ring implements RNS-accelerated modular arithmetic operations for polynomials,
// including: RNS basis extension; RNS rescaling; number theoretic transform (NTT);
// uniform, Gaussian and ternary sampling.
...
// Ring 是一种结构，它保留了在这个环中表示的多项式上运行所需的所有变量
type Ring struct {
	...
	NttPsi    [][]uint64 //powers of the 2N-th primitive root in Montgomery form (in bit-reversed order)
...
// N must be a power of two larger than 8. Moduli should be
// a non-empty []uint64 with distinct prime elements. For the Ring instance to support NTT operation,
// these elements must also be equal
...
// setParameters initializes a *Ring by setting the required precomputed values
//(except for the NTT-related values, which are set by the
...
// genNTTParams checks that N has been correctly initialized,
// and checks that each modulus is a prime congruent to 1 mod 2N (i.e. NTT-friendly).
// Then, it computes the variables required for the NTT.
// The purpose of ValidateParameters is to validate that the moduli allow the NTT,
// and to compute the NTT parameters.
...
// MarshalBinary encodes the target ring on a slice of bytes.   在字节片上编码目标环
...
// GetBredParams returns the Barret reduction parameters of the Ring.   返回环的拉贝特还原参数
func (r *Ring) GetBredParams() [][]uint64 {
	return r.BredParams
}

// GetMredParams returns the Montgomery reduction parameters of the Ring.  返回环的蒙哥马利还原参数
