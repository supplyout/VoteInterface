...
// 系数乘以多项式形式乘法，将结果输出到p3中

// 将p1和p2多项式进行加和，将结果存储到p3上
func (r *Ring) MulCoeffsMontgomeryMy(p1 *Poly, p2 int, p3 *Poly) {
	for i, qi := range r.Modulus {
		p1tmp, p3tmp := p1.Coeffs[i], p3.Coeffs[i]
		mredParams := r.MredParams[i]
		...
		for j := uint64(0); j < r.N; j = j + 8 {
			...
			x := (*[8]uint64)(unsafe.Pointer(&p1tmp[j]))
			//y := (*[8]uint64)(unsafe.Pointer(&p2tmp[j]))
			p2tmp := uint64(math.Mod(float64(p2), float64(qi)))
			p2tmp = MForm(2, qi, r.BredParams[i])
			z := (*[8]uint64)(unsafe.Pointer(&p3tmp[j]))

			z[0] = MRed(x[0], p2tmp, qi, mredParams)
			z[1] = MRed(x[1], p2tmp, qi, mredParams)
			z[2] = MRed(x[2], p2tmp, qi, mredParams)
			z[3] = MRed(x[3], p2tmp, qi, mredParams)
			z[4] = MRed(x[4], p2tmp, qi, mredParams)
			z[5] = MRed(x[5], p2tmp, qi, mredParams)
			z[6] = MRed(x[6], p2tmp, qi, mredParams)
			z[7] = MRed(x[7], p2tmp, qi, mredParams)
		}
	}
}
...
// coeffs[2], 0, 0, ...
// 将这个多项式转换成蒙哥马利表示形式
func (r *Ring) MFormArray(coeffs []uint64, p2 *Poly) {
	zeros := make([][]uint64, len(r.Modulus))
	for i := 0; i < len(r.Modulus); i++ {
		zeros[i] = make([]uint64, r.N)
		bredParams := r.BredParams[i]
		qi := r.Modulus[i]
		zeros[i][0] = MForm(coeffs[i], qi, bredParams)
	}
	p2.SetCoefficients(zeros)
...
// 将一个数组转换成蒙哥马利表示形式的多项式
func (r *Ring) MFormArrayUint(coeffs []*uint64, pol *Poly) {
	for i, qi := range r.Modulus {
		p2tmp := pol.Coeffs[i]
		bredParams := r.BredParams[i]
		for j := uint64(0); j < r.N; j = j + 8 {

			x := (*[8]uint64)(unsafe.Pointer(&coeffs[j]))
			z := (*[8]uint64)(unsafe.Pointer(&p2tmp[j]))
...
// 将一个int类型的数组转换成一个MFORM形式的多项式
func (r *Ring) MFormArrayInt(coeffs []int, pol *Poly) {

	for i, qi := range r.Modulus {
		p1tmp := make([]uint64, r.N)
...
// 将p1表示的数组转换成原本形式的，将结果存储在p2中
func (r *Ring) InvMFormArray(p1 []uint64, p2 []*uint64) {
	qi := r.Modulus[2]
	mredParams := r.MredParams[2]
	for j := uint64(0); j < r.N; j = j + 8 {
